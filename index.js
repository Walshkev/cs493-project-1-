...
app.post('/businesses', (req, res) => {
    const { name, streetAddress, city, state, zipCode, phoneNumber, category, subcategories } = req.body;
    if (!name || !streetAddress || !city || !state || !zipCode || !phoneNumber || !category) {
...
app.delete('/businesses/:id/reviews/:reviewId', (req, res) => {
    ...
    const businessData = businesses.get(id);
    if (businessData) {
        const reviewIndex = businessData.reviews.findIndex(review => review.id === reviewId);
        if (reviewIndex === -1) {
        ...
        res.status(204).json({ "message": `Review ${reviewId} deleted from businesses ${id}` });
    } else {
        res.status(404).json({ "error": `businesses ${id} not found` });
    }
});
...
app.delete('/businesses/:id/photos/:photoId', (req, res) => {
    ...
    const businessData = businesses.get(id);
    if (businessData) {
        const photoIndex = businessData.photos.findIndex(photo => photo.id === photoId);
        if (photoIndex === -1) {
        ...
        res.status(204).json({ "message": `photo ${photoId} deleted from businesses ${id}` });
    } else {
        res.status(404).json({ "error": `businesses ${id} not found` });
    }
});
...
app.post('/businesses/:id/reviews', (req, res) => {
    ...
    if (starRating === undefined || dollarRating === undefined) {
        return res.status(400).json({ "error": "Star rating and dollar rating are required" });
    }
    if (starRating < 0 || starRating > 5 || dollarRating < 1 || dollarRating > 4) {
        return res.status(400).json({ "error": "Star rating must be between 0 and 5, and dollar rating must be between 1 and 4" });
    }
    ...
    if (businessData) {
        ...
        res.status(201).json({ "message": `Review added to businesses ${id}`, "review": reviewEntry });
    } else {
        res.status(404).json({ "error": `businesses ${id} not found` });
    }
});
...
app.put('/businesses/:id', (req, res) => {
    ...
    if (!name && !streetAddress && !city && !state && !zipCode && !phoneNumber && !category && !subcategories) {
    ...
    res.status(200).json({ "message": `Business ${id} updated`, "business": businessData.details });
});
...
app.post('/businesses/:id/photos', (req, res) => {
    ...
    if (businessData) {
        ...
        res.status(201).json({ "message": `Photo added to businesses ${id}`, "photo": photoEntry });
    } else {
        res.status(404).json({ "error": `businesses ${id} not found` });
    }
});
...
app.put('/businesses/:id/photos/:photoId', (req, res) => {
    ...
    if (businessData) {
        ...
        res.status(200).json({ "message": `Photo ${photoId} updated for businesses ${id}`, "photo": businessData.photos[photoIndex] });
    } else {
        res.status(404).json({ "error": `Businesses ${id} not found` });
    }
});
...
app.put('/businesses/:id/reviews/:reviewId', (req, res) => {
    ...
    if ( starRating == undefined || dollarRating ==undefined) {
        ...
        if (starRating < 0 || starRating > 5 || dollarRating < 1 || dollarRating > 4) {
            return res.status(400).json({ "error": "Star rating must be between 0 and 5, and dollar rating must be between 1 and 4 please" });
        }
        else{
            if (review == undefined){
                review = null;
            ...
            const businessData = businesses.get(id);
            if (businessData) {
                const reviewIndex = businessData.reviews.findIndex(r => r.id === reviewId);
                if (reviewIndex === -1) {
                    return res.status(404).json({ "error": `Review ${reviewId} not found` });
                }
                ...
                res.status(200).json({ "message": `Review ${reviewId} updated for businesses ${id}`, "review": businessData.reviews[reviewIndex] });
            }
            else {
                res.status(404).json({ "error": `businesses ${id} not found` });
            }
